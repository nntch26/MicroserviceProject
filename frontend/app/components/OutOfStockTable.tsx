   ...
   useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const lowStockItems = resInventory.filter((item: InventoryProducts) => item.quantity_in_stock <= 0);
          ...
          {filteredInventory.length === 0 ? (
            <tr>
              <td colSpan={showActions ? 8 : 7} className="px-6 py-4 text-center text-gray-500">
                No products out of stock found
              </td>
            </tr>
          ) : (
            filteredInventory.map((item) => {
              ...
              return (
                <tr key={item._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap bg-red-100 text-red-800 text-center">out_of_stock</td>
