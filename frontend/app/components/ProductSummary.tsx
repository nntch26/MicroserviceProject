...
export function ProductSummary() {
  ...
  const fetchData = async () => {
    const productData = await fetchInventoryProduct();
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <SummaryCard
        ...
        value={products.filter((p) => p.quantity_in_stock < 10 && p.quantity_in_stock != 0).length.toString()}
        icon={<AlertTriangle size={24} className="text-amber-500" />}
      />

      <SummaryCard
        title="Out of Stock"
        value={products.filter((p) => p.quantity_in_stock === 0).length.toString()}
